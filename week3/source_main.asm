...
;======================================================================================
		...
		MOV		@TIMER1PRD,#4000	;設定Timer1中斷週期 4000*1/2000M = 20us
...
;		MOV		@ADCAINLTRIM1,#0xFFFF0000
		...
		MOV   	@ADCACTL2,#0x0006			;ADCCLOCK = Input clock/4
		MOV   	@ADCACTL1,#0x0004			;INTPULSEPOS
		OR		@ADCACTL1,#0x0080			;power up ADC
		MOV   	@AR0,#20000
DELAY_1ms:
		NOP
		BANZ  DELAY_1ms,AR0--				;1ms delay for power up ADC
		MOV   	@ADCASOC2CTL,#0x01FF		;SOC1  選channel A0 ACQPS 33cycle
		MOV   	@ADCASOC2CTL+1,#0x0001		;SOC Trigger Source Select
		MOV		@ADCAINTSEL1N2,#0x0022		;ENABLE ADCINT1 , EOC0 is trigger for ADCINT1
...
;0:Input , 1:Output , defalt is 0:Input
		...
		MOV    	@GPADIR,#0x100F      	;0~15;GPIO0 is output
